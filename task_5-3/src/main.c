...
#define BASEDUTYCYCLEVALUE 51 // Fan speed incremented by 20% on every button press -> (MAX Value of unit_8)*0.25
...
void DisplayFanSpeed(void)
{
	...
	uint16_t fanSpeedMedian = fanspeed_getFiltered();
	fprintf(lcdout, "Fan Speed: %d RPM\n", fanSpeed);
	fprintf(lcdout, "PWM Value: %d\n", dutyCycleValue);
	fprintf(lcdout, "Filtered Value: %d RPM\n", fanSpeedMedian);
}
