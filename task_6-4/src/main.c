...
/**
 * Event handler to dispatch the rotary clockwise rotation event to the fsm
 */
static void clockwiseEventHandler()
{
...
/**
 * Event handler to dispatch the rotary anti-clockwise rotation event to the fsm
 */
static void counterClockwiseEventHandler()
{
    Event e = {.signal = ROTARY_COUNTER_CLOCKWISE};
	// Dispatch button press event to the fsm
	fsm_dispatch(&alarmClock_fsm, &e);
}
...
int main(void)
{
    ...
    // Rotary callback setting
    button_setRotaryClockwiseCallback(clockwiseEventHandler);
    button_setRotaryCounterClockwiseCallback(counterClockwiseEventHandler);
